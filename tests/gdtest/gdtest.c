    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            if (p1 == p2) {
                ...
                if (abs(ga1 - ga2) > alpha_threshold) {
                    ...
                    if (a1 == 0 && a2 == 0) {
                        ...
                        is_diff = pixel_diff_yiq(r1, g1, b1, r2, g2, b2) > max_delta;
                    } else {
                        /* composite against white and black, take best case */
                        double delta_white = pixel_diff_yiq(
                            blend(r1, a1, 255), blend(g1, a1, 255), blend(b1, a1, 255),
                            blend(r2, a2, 255), blend(g2, a2, 255), blend(b2, a2, 255)
                        );
                        double delta_black = pixel_diff_yiq(
                            blend(r1, a1, 0), blend(g1, a1, 0), blend(b1, a1, 0),
            ...
            if (is_diff) {
                result_ret->pixels_changed++;
                if (buf_diff) {
                    gdImageSetPixel(buf_diff, x, y, gdTrueColorAlpha(255, 0, 0, 0));
                }
            } else if (buf_diff) {
                /* faded original for spatial context */
                gdImageSetPixel(buf_diff, x, y, gdTrueColorAlpha(
                    gdTrueColorGetRed(p1),
                    gdTrueColorGetGreen(p1),
                    gdTrueColorGetBlue(p1),
                    102
                ));
            }
        }
